...
namespace System
{
    public class cUdpSock
    {
        ...
        private void udpListen()//
        {
            try
            {
                ...
                while (!isOutClass)
                {
                    ...
                    {
                        mDataReciveByte(null, new RecieveByteArgs(ie.Address.ToString(),ie.Port, b));
                    }
                    string s = Encoding.GetEncoding("gb2312").GetString(b, 0, b.Length);
                    if (mDataReciveString != null)
                    {
                        mDataReciveString(null, new RecieveStringArgs(ie.Address.ToString(),ie.Port, s));
                    }
                }
            }
            catch
        ...
        {
            string localIDName = "EMAC1";
            object value = Registry.GetValue(string.Format(@"HKEY_LOCAL_MACHINE\Comm\{0}\Linkage", "Tcpip"), "Bind", "EMAC1");
            if (value != null)
            {
        ...
        public static IPAddress LoaclIp()
        {
            ...
            if (NewMideaProgram.cMain.isComPuter)
            {
                foreach (IPAddress ipAdd in Dns.GetHostAddresses(Dns.GetHostName()))
                {
                    localIpGet = ipAdd.ToString();
                    tempLastIp = ipAdd.ToString().Substring(ipAdd.ToString().LastIndexOf(".") + 1);
                    if (ipAdd.ToString().IndexOf("200") == 0 && ipAdd.ToString().IndexOf("10")==0 ||
                        ipAdd.ToString().IndexOf("192") == 0)
                    {
                        return ipAdd;
                    }
                }
        ...
        {
            Registry.SetValue(string.Format(@"HKEY_LOCAL_MACHINE\Comm\{0}\Parms\TcpIp", NetBoardName()), "IpAddress", ipAddress);
        }
    }
    public class RecieveStringArgs : EventArgs
    {
        ...
        public readonly int RemotPort;
        public RecieveStringArgs(string mRemostHost,int remotPort, string mReadStr)
        {
            RemotPort = remotPort;
    ...
    public class RecieveByteArgs : EventArgs
    {
        ...
        public readonly int RemotPort;
        public RecieveByteArgs(string mRemostHost,int remotPort, byte[] mReadByte)
        {
            RemotPort = remotPort;
            RemostHost = mRemostHost;
            ReadByte = mReadByte;
